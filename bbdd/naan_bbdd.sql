...
--
-- Servidor: localhost
-- Tiempo de generación: 21-02-2026 a las 11:55:20
...
CREATE TABLE `carrito` (
  ...
  `estado` enum('activo','comprado') COLLATE utf8mb4_unicode_ci DEFAULT 'activo'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TRIGGER `trg_metodo_predeterminado_update` BEFORE UPDATE ON `metodo_pago` FOR EACH ROW BEGIN
    IF NEW.es_predeterminado = TRUE THEN
        ...
        SET es_predeterminado = FALSE
        WHERE id_usuario = NEW.id_usuario
        AND id_metodo_pago <> NEW.id_metodo_pago;
    END IF;
END
$$
DELIMITER ;
...
ALTER TABLE `favorito`
  ...
  ADD KEY `idx_favorito_producto` (`id_producto`);
...
ALTER TABLE `metodo_pago`
  ...
  ADD KEY `idx_metodo_usuario` (`id_usuario`);
...
ALTER TABLE `pedido`
  ...
  ADD KEY `idx_pedido_pago` (`id_metodo_pago`),
  ADD KEY `idx_pedido_direccion` (`id_direccion`);
